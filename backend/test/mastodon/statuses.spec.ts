...
describe('Mastodon APIs', () => {
	describe('statuses', () => {
		...
		test('favourite status sends Like activity', async () => {
			...
			const res = await statuses_favourite.handleRequest(db, 'mastodonid1', connectedActor, userKEK, queue)
			assert.equal(res.status, 200)
		...
		test('favourite records in db', async () => {
			...
			const res = await statuses_favourite.handleRequest(db, note.mastodonId!, connectedActor, userKEK, queue)
			assert.equal(res.status, 200)
